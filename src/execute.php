...
ini_set('memory_limit', '12296M');
    ...
    foreach ($tideman_collection_list as $name => $path) :
        ...
        $results[$name]['condorcetFormatVotes']['implicitRankingEvaluationOfVotes'] .= CondorcetElectionFormat::createFromElection(election: $election, aggregateVotes: true, includeTags: false, inContext: true);

        // Official conversion to .cvotes
        $results[$name]['condorcetFormatVotes']['officialCvotesConversion'] = $specifications . $numberOfVotes;
        $results[$name]['condorcetFormatVotes']['officialCvotesConversion'] .= CondorcetElectionFormat::createFromElection(election: $election, aggregateVotes: !str_starts_with($name, 'D') ? true : false, includeTags: true, inContext: false);
        ...
        $results[$name]['condorcetFormatVotes']['explicitRankingEvaluationOfVotes'] .= CondorcetElectionFormat::createFromElection(election: $election, aggregateVotes: true, includeTags: false, inContext: false);
